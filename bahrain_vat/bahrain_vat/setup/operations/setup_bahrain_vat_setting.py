...
def add_permissions():
	...
	frappe.db.set_value('Report', 'BAHRAIN VAT', 'disabled', 0)
def create_bahrain_vat_setting(self, method):
    ...
    if len(company_list) == 1 and len(bahrain_vat_setting) == 0:
        make_custom_fields()
        file_path = os.path.join(os.path.dirname(__file__), '..', 'data', 'bahrain_vat_settings.json')
        with open(file_path, 'r') as json_file:
            account_data = json.load(json_file)
        

        # Creating BAHRAIN VAT Setting
        bahrain_vat_setting = frappe.get_doc({
            'doctype': 'BAHRAIN VAT Setting',
            'company': self.name
        })
        ...
        for data in account_data:
            if data['type'] == 'Sales Account':
                ...
                for row in data['accounts']:
                    item_tax_template = row['item_tax_template']
                    account = row['account']
                    bahrain_vat_setting.append('bahrain_vat_purchase_accounts', {
                        'title': row['title'],
                        'item_tax_template': f'{item_tax_template} - {self.abbr}',
                        'account': f'{account} - {self.abbr}'
                    })

        bahrain_vat_setting.save()
...
def make_custom_fields():
	...
	sales_invoice_fields = [
			...
			dict(fieldname='bahrain_einv_qr', label='BAHRAIN E-Invoicing QR',
...
def update_regional_tax_settings(country, company):
	create_bahrain_vat_setting(company)
